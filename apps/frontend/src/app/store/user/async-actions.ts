...
export const addCertificateAction = createAsyncThunk<IUser, FormData>('user/add-certificate', async (formData) => {
  const { data } = await userApi.addCertificate(formData);
  return data;
});

export const updateCertificateAction = createAsyncThunk<IUser, FormData>('user/update-certificate', async (formData) => {
  const { data } = await userApi.updateCertificate(formData);
  return data;
});

export const deleteCertificateAction = createAsyncThunk<IUser, string>('user/delete-certificate', async (src) => {
  const { data } = await userApi.deleteCertificate(src);
  return data;
});

export const toFriendAction = createAsyncThunk<void, string>('user/to-friend', async (id) => {
  await userApi.toFriend(id);
});

export const removeFriendAction = createAsyncThunk<void, string>('user/remove-coach-friend', async (id) => {
  await userApi.removeFriend(id);
});

export const subscribeAction = createAsyncThunk<void, string>('user/subscribe', async (id) => {
